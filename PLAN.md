...
- 2026-07-01：切換播放清單後按鈕狀態已重製，但再次點擊「反轉播放」會跳到前一個播放清單最後一支。原因是 YouTube 單頁切換時舊播放清單 DOM 可能短暫殘留，播放清單解析只看 `v=`，沒有確認 `list=`；已改為優先只使用連結中 `list` 等於目前播放清單 ID 的影片項目。
- 2026-07-01：新增不可播放影片自動跳過初版。反轉播放啟用時，若播放器區域出現會員限定、等待直播、私人影片、已刪除或無法播放等狀態文字，會自動導向反轉佇列中的下一支影片。
- 2026-07-01：測試「即將開始直播」時，YouTube 沒有顯示可掃描的狀態文字，因此未自動跳過。已新增第二層偵測，讀取目前影片的 YouTube player response，透過 `playabilityStatus` 與直播開始時間判斷會員限定、不可播放與尚未開始的直播。
